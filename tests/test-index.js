...
function allTests() {
    ...
    it('posts object key and value', (done) => {
        superagent.post('https://mydoc-code-test.herokuapp.com//api/v1/object')
            .send(testData1)
            .end((err, res) => {
    ...
    it('gets latest value when no timestamp is given', (done) => {
        ...
        superagent.get('https://mydoc-code-test.herokuapp.com//api/v1/object/' + myKey)
            .end((err, res) => {
                expect(err).toBe(null)
                expect(typeof res.body).toBe('object')
                expect(res.body.key).toBe(Object.keys(testData1)[0])
    ...
    it('gets value immediately less than or equal to given timestamp', (done) => {
        ...
        superagent.get('https://mydoc-code-test.herokuapp.com/timestamp/api/v1/object/' + myKey + '/' + thirdTime)
            .end((err, res) => {
                expect(err).toBe(null)
                expect(typeof res.body).toBe('object')
                expect(res.body.key).toBe(Object.keys(testData1)[0])
